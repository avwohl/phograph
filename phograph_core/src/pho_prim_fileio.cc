...
void register_fileio_prims() {
    ...
    r.register_prim("file-delete", 1, 1, [](const std::vector<Value>& in) -> PrimResult {
        ...
        return PrimResult::success(Value::boolean(unlink(in[0].as_string()->c_str()) == 0));
    });
    ...
    r.register_prim("file-size", 1, 1, [](const std::vector<Value>& in) -> PrimResult {
        ...
        struct stat st;
        if (stat(in[0].as_string()->c_str(), &st) != 0)
    ...
    r.register_prim("dir-list", 1, 1, [](const std::vector<Value>& in) -> PrimResult {
        ...
        DIR* dir = opendir(in[0].as_string()->c_str());
        if (!dir) return PrimResult::fail_with(Value::error("dir-list: could not open directory"));
        std::vector<Value> entries;
        struct dirent* ent;
        while ((ent = readdir(dir)) != nullptr) {
            if (strcmp(ent->d_name, ".") == 0 || strcmp(ent->d_name, "..") == 0) continue;
            entries.push_back(Value::string(ent->d_name));
        }
        closedir(dir);
    ...
    r.register_prim("dir-create", 1, 1, [](const std::vector<Value>& in) -> PrimResult {
        ...
        return PrimResult::success(Value::boolean(mkdir(in[0].as_string()->c_str(), 0755) == 0));
    });
    ...
    r.register_prim("path-join", 1, 1, [](const std::vector<Value>& in) -> PrimResult {
        ...
        for (size_t i = 0; i < lst->size(); i++) {
            ...
            if (i > 0 && !result.empty() && result.back() != '/') result += '/';
    ...
    r.register_prim("file-is-directory?", 1, 1, [](const std::vector<Value>& in) -> PrimResult {
        ...
        struct stat st;
        if (stat(in[0].as_string()->c_str(), &st) != 0) return PrimResult::success(Value::boolean(false));
        return PrimResult::success(Value::boolean(S_ISDIR(st.st_mode)));
    });
    ...
    r.register_prim("temp-dir", 0, 1, [](const std::vector<Value>&) -> PrimResult {
        ...
        if (!tmp) tmp = "/tmp";
        return PrimResult::success(Value::string(tmp));
    });

    // cwd: -> string
    r.register_prim("cwd", 0, 1, [](const std::vector<Value>&) -> PrimResult {
        char buf[4096];
        if (!getcwd(buf, sizeof(buf))) return PrimResult::fail_with(Value::error("cwd: failed"));
