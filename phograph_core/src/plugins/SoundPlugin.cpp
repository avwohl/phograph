 ...
 *
 * Architecture:
 *   Engine thread -> ring buffer -> AudioQueue callback -> speakers
 *
 * The ring buffer is lock-free (single producer, single consumer)
 * using atomic head/tail indices.
 *
 * Derived from iospharo SoundPlugin (source-level sharing).
 */
...
static constexpr int kRingCapacity = 64 * 1024; // 64KB
static uint8_t gRingBuffer[kRingCapacity];
static std::atomic<int> gRingHead{0};
static std::atomic<int> gRingTail{0};
...
static constexpr int kBufferByteSize = 4096;
...
#else
// Non-Apple stubs
bool soundInit(int, bool) { return false; }
void soundStop(void) {}
